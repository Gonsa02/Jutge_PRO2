...
void Cjt_estudiants::afegir_estudiant(const Estudiant& est) {
  ...
  vest[i+1] = est;
  ++nest;
  //El imax s'ha desplaçat al insertar el nou Estudiant
  if (i+1 <= imax) ++imax;
  if (est.te_nota()) {
    if (imax == -1) imax = i+1;
...
void Cjt_estudiants::modificar_estudiant(const Estudiant& est) {
  ...
  if (est.te_nota()) {
    if (imax == -1) imax = i;
    else if (est.consultar_nota() > vest[imax].consultar_nota()) imax = i;
    else if (est.consultar_nota() == vest[imax].consultar_nota() and est.consultar_DNI() < vest[imax].consultar_DNI()) imax = i;
  }
  if (i == imax or est.consultar_DNI() == vest[imax].consultar_DNI()) recalcular_posicio_imax();
}    
...
void Cjt_estudiants::modificar_iessim(int i, const Estudiant& est) {
  ...
  if (i-1 == imax or est.consultar_DNI() == vest[imax].consultar_DNI()) recalcular_posicio_imax();
}
...
{
	int i = cerca_dicot(vest,0,nest-1,dni);
	if (i == -1) throw PRO2Excepcio("El conjunt no conte aquest estudiant");
	bool is_imax = i == imax;
	if (i < imax) --imax;
	++i;
	//Desplacem l'estudiant al final del vector INICIALITZAT
	while (i < nest) vest[i-1] = vest[i], ++i;
	--nest;
	if (is_imax) recalcular_posicio_imax();
}
...
{
    imax = 0;
    while(imax != nest and not vest[imax].te_nota()) ++imax;

    if (imax == nest) imax = -1;
    else {
	for (int i = imax+1; i < nest; ++i) {
	    if (vest[i].te_nota()) {
		if (vest[i].consultar_nota() > vest[imax].consultar_nota()) imax = i;
		else if (vest[i].consultar_nota() == vest[imax].consultar_nota() and vest[i].consultar_DNI() < vest[imax].consultar_DNI()) imax = i;
	    }
	}
    }
}
